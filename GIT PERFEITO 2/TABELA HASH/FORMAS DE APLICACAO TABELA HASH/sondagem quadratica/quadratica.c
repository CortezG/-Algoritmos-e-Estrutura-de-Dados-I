...
int hash_sondagem_quadratica(int chave, int i, int tamanho) {
    return (chave + i * i) % tamanho;
}

void inserir(TabelaHash *tabela, int chave) {
    int i = 0;
    int indice = hash_sondagem_quadratica(chave, i, tabela->tamanho);

    while (tabela->tabela[indice] != 0) {
        i++;
        indice = hash_sondagem_quadratica(chave, i, tabela->tamanho);
    }
