...
int hash_sondagem_linear(int chave, int i, int tamanho) {
    return (chave + i) % tamanho;
}

void inserir(TabelaHash *tabela, int chave) {
    int i = 0;
    int indice = hash_sondagem_linear(chave, i, tabela->tamanho);

    while (tabela->tabela[indice] != 0) {
        i++;
        indice = hash_sondagem_linear(chave, i, tabela->tamanho);
    }
